...
class GA
{
    ...
    static private $distanceMatrix = [];
    static private $timeMatrix = [];
    ...
    {
        $child = new Tour(self::getDistanceMatrix(), self::getTimeMatrix());
    ...
    static public function generateDistanceMatrix() {
        $cities = TourManager::getCities();

        foreach ($cities as $city1) {
            foreach ($cities as $city2) {
                $aDist = $city1->distanceTo($city2);

                self::$distanceMatrix[$city1->getId()][$city2->getId()] = $aDist[0];
                self::$timeMatrix[$city1->getId()][$city2->getId()] = $aDist[1];
            }
        }
    }

    static public function getDistanceMatrix() {
        return self::$distanceMatrix;
    }

    static public function getTimeMatrix() {
